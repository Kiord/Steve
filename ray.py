...
def hit_sphere(ray: Ray, sphere: Sphere, t_min, t_max):
    ...
    if discriminant > 0:
        ...
        if t_min < root < t_max:
            ...
            rec.normal = outward
            rec.front_face = 1 if ray.direction.dot(outward) < 0 else 0
            if rec.front_face == 0:
...
def hit_scene(ray:Ray, scene: ti.template(), t_min, t_max): # type: ignore
    ...
    for i in range(scene.num_spheres[None]):
        hit, temp_rec = hit_sphere(ray, scene.spheres[i], t_min, closest)
        if hit:
            closest = temp_rec.t
            final_rec = temp_rec
            hit_anything = True
    ...
    for i in range(scene.num_planes[None]):
        hit, temp_rec = hit_plane(ray, scene.planes[i], t_min, closest)
        if hit:
            closest = temp_rec.t
            final_rec = temp_rec
            hit_anything = True
